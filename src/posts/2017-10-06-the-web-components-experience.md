...
For example, if we remove the `title` ref element in the previous code and then reinsert it, an error would be thrown when referencing `this.refs.title` since the original node doesn't exist anymore.
