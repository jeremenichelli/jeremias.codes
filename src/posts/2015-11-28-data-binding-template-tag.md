...
excerpt: There's a discussion taking place in the front end development industry right now about frameworks. No matter which side you are on, the truth is we sometimes forget all the things the web platform has available for us to achieve some complex requirements.
---
...
Cross browsing support is a good argument, but modern ones are still the most used so it makes more sense to start developing from scratch for those environments and then maybe start adding fallbacks for others.

## Data binding

Just like DOM selection for jQuery, data binding is one of the reasons why developers tend to use an MVC framework, but again it comes with other stuff that might be useful but you might not need. There are a lot of small data binding alternatives that will perform equally or better than these libraries.

Last year I wrote [a small data binding library](//github.com/jeremenichelli/mnster), nothing complicated. It just covers the basic needs when inserting data dynamically with JavaScript and a super easy API to add new bindings in seconds.
...
I've just called the `view` method, passing an element and then an object with the context name I'll use in my attributes and an object that holds the data to bind.

## Views and data

In more complex applications it is not good to have everything inside a JavaScript file. Most frameworks have already defined structures that use _.html_ files for views and usually get the data from a server request.
...
You can still access its content and make modifications, but I think the right way to use it is to import its content in a new variable and apply the changes into that cloned node.
...
/*
 * Templates object
 * for convention use <TYPE>-template notation to name them
 */
var templates = {
...
/*
 * Views object
 * for convention use <TYPE>-view notation to name them
 */
var views = {
...
Now we can easily call the `build` function to bind the data and insert the result in the DOM.
