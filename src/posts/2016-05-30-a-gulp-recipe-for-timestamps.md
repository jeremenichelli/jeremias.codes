...
We don't want to generate new timestamps in these cases because we will force returning users to download assets again when they haven't really changed.
...
Jekyll gives us a _\_data_ folder to place files whose content can be later accessed by its templating engine, so we're going to create a new _timestamps.json_ file inside this directory.
...
The method can be easily used in other tasks by changing the string passed to the _updateTimestamp_ method.
