...
If we don't place the `@Input` decorator before the property, not only the parent component won't be able to pass a value to it, but it will also generate an error at compile time.
